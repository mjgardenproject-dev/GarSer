...
const PROMPT_MODULES: Record<AnalysisService, PromptModule> = {
  ...
  'Desbroce de malas hierbas': {
    ...
    serviceRules: [
      'SERVICE RULES:',
      '- Analyze only weed-covered ground that clearly belongs to the clearing job.',
      '',
      'PROCEDURE (follow in order):',
      '1. Cross-reference all photos and deduplicate repeated angles using persistent anchors (walls, fences, trees, buildings).',
      '2. Locate scale references near the plot: person ~1.70 m, standard door ~2.0 m, garden fence ~1.2 m, car ~4.5 m long, wheelie bin ~1.0 m. Record the reference used in "referencia_escala".',
      '3. Measure the consolidated weed-covered area (m2) following the plot outline. Exclude paved areas, buildings and maintained lawn.',
      '4. Classify estado_malas_hierbas using the definitions below.',
      '5. Assign per-field confidence using the calibration below.',
      '',
      'STATE DEFINITIONS (observable criteria):',
      '- "normal": low soft weeds below ~30 cm with low density; bare ground visible between plants.',
      '- "dificultad_media": non-woody weeds ~30-80 cm or medium/high density covering most of the ground.',
      '- "dificultad_alta": woody brush, brambles, cane-like stems, or dense mass above ~80 cm requiring heavy machinery.',
      '- estado_malas_hierbas must be exactly "normal", "dificultad_media" or "dificultad_alta". If vegetation detail is not visible, use null and lower estado_confidence.',
      '',
      'PLAUSIBLE AREA RANGE:',
      '- Residential clearing plots measure between 1 and 10000 m2. If your estimate exceeds 10000 m2, set nivel_analisis to 2 or 3 and add "AMBIGUOUS_SIZE" to observaciones instead of forcing the value.',
      '',
      'CONFIDENCE CALIBRATION (applies to superficie_confidence and estado_confidence):',
      '- 0.9-1.0: directly measurable in sharp photos with a clear scale reference and the full plot visible.',
      '- 0.6-0.89: inferred with partial support (no clear scale reference, plot partially cropped).',
      '- Below 0.6: weak assumption; the system will ask the client to confirm this field.',
      '- Without a scale reference, superficie_confidence MUST be below 0.9.',
      '',
  ...
  'Servicios fitosanitarios': {
    ...
    serviceRules: [
      ...
      '',
      'PROCEDURE (follow in order):',
      '1. Cross-reference all photos and deduplicate repeated angles using persistent anchors (walls, paths, pools, buildings).',
      '2. Locate scale references: person ~1.70 m, standard door ~2.0 m, garden fence ~1.2 m, patio tile ~0.4 m.',
      '3. For each vegetation family INSIDE the requested scope, quantify using the units of its metric (m2, ml or units).',
      '4. Classify sizes using the definitions below (they select the gardener price tier, so apply them strictly).',
      '5. If a count or measure is uncertain, report the LOWER plausible value and add AMBIGUOUS_COUNT or AMBIGUOUS_SIZE.',
      '',
      'SIZE DEFINITIONS (same cut-offs as the gardener pricing configurator):',
      '- Hedges: "seto_bajo_medio_ml" = hedge lines below ~2.5 m tall; "seto_alto_ml" = 2.5-5 m tall.',
      '- Palms (shower treatment): "peq" < 3.5 m total height; "med" 3.5-8 m; "alta" > 8 m.',
      '- Trees: "peq" < 3 m tall; "med" 3-6 m; "gran" > 6 m.',
      '- Plants dominant size: "pequenas" < 0.5 m; "medianas" 0.5-1.5 m; "grandes" 1.5-2 m.',
      '',
      'PLAUSIBLE RANGES (residential):',
      '- cesped_m2 <= 2000; plantas_superficie_calculada_m2 <= 500; hedge lines <= 200 ml; palms <= 30 units; trees <= 50 units.',
      '- If an estimate exceeds its range, report the maximum of the range and add "AMBIGUOUS_SIZE" to observaciones_ia instead of inventing a larger value.',
    ],
    outputSchema: [
      '{',
      '  "razonamiento_transversal": {',
