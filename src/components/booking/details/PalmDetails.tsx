...
export const PalmDetails = ({
    idx,
    group,
    allPhotos,
    isZoneAnalyzing,
    palmUploads,
    handlePalmFileSelect,
    togglePalmPhotoSelection,
    removePalmPhoto,
    removePalmGroup,
    analyzePalmGroup,
    isAnalyzed,
    ...
    if (isZoneAnalyzing) {
        return <AnalysisLoadingAnimation key={group.id} message="Analizando palmeras..." />;
    }
    ...
    if ((group as any).isFailed || group.analysisLevel === 3) {
        return (
            <AnalysisFailedCard 
                ...
                onReanalyze={() => analyzePalmGroup(group.id)} 
            />
        );
    }
    ...
    return (
        <div key={group.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
                ...
                <button onClick={() => removePalmGroup(group.id)} className="text-gray-400 hover:text-red-500 p-2 hover:bg-red-50 rounded-lg transition-colors">
            ...
            <div className="mb-4">
                <ZonePhotoGallery
                    photos={allPhotos}
                    uploadingIndices={palmUploads[group.id]}
                    selectedIndices={group.selectedIndices}
                    analyzedIndices={group.analyzedIndices}
                    isAnalyzing={isZoneAnalyzing}
                    isAnalyzed={isAnalyzed}
                    onToggleSelection={(i: number) => togglePalmPhotoSelection(group.id, i)}
                    onRemovePhoto={(i: number) => removePalmPhoto(group.id, i)}
                    onAddPhotos={(e: any) => handlePalmFileSelect(group.id, e)}
            ...
            <div className="mt-2">
                <ZoneActionButton
                    onClick={() => analyzePalmGroup(group.id)}
                    isAnalyzing={isZoneAnalyzing}
                    isAnalyzed={isAnalyzed}
            ...
            {isAnalyzed && (
                <div className="mt-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <ServiceResultCard
                        ...
                        metrics={[
                            { label: 'Especie', value: group.species || 'Desconocida' },
                            { label: 'Altura', value: group.height || 'Normal' }
                        ]}
