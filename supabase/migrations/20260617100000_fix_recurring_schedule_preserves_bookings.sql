-- Fix generate_recurring_slots: after regenerating availability from the recurring
-- schedule template, re-mark slots that belong to confirmed/in-progress bookings
-- as is_available = false so they cannot be double-booked.
--
-- Previously, force_regenerate=true deleted all future availability rows and
-- re-inserted them as is_available=true, inadvertently exposing already-confirmed
-- booking slots back to the booking engine.
...
BEGIN
  ...
  -- The loop above may have reset is_available=true for slots that were
  -- already occupied by a confirmed booking.
