...
export type FumigationRange = '0-50' | '50-200' | '200+';

export interface FumigationPricingConfig {
  type_prices: Record<string, Partial<Record<FumigationRange, number>>>; 
  waste_removal: {
      percentage: number;
  };
  selected_types?: FumigationType[];
}

const FUMIGATION_TYPES: FumigationType[] = [
  'Insecticida',
  'Fungicida',
  'Herbicida'
];

const EMPTY_CONFIG: FumigationPricingConfig = {
  type_prices: {},
...
const FumigationPricingConfigurator: React.FC<Props> = ({ value, initialConfig, onChange, onSave }) => {
  ...
  const addType = (type: FumigationType) => {
  ...
  const removeType = (type: FumigationType) => {
      const currentSelected = config.selected_types || [];
  ...
  const handlePriceChange = (type: FumigationType, range: FumigationRange, newPrice: number) => {
  ...
  const handleSave = async (e?: React.MouseEvent) => {
    ...
    selected.forEach(type => {
        const ranges: FumigationRange[] = ['0-50', '50-200', '200+'];
        ranges.forEach(r => {
             // @ts-ignore
    ...
    if (onSave) {
      try {
        ...
        console.error('Error saving fumigation config:', error);
      } finally {
        setIsSaving(false);
      }
    }
  ...
  const renderPriceInput = (type: FumigationType, range: FumigationRange, placeholder: string) => {
     // @ts-ignore
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
        <div className="flex items-center gap-2">
            <h3 className="font-semibold text-gray-900 text-lg">
                Configuración de fumigación (IVA incluido)
            </h3>
                    ...
                    <>
                        ...
                        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[90vw] max-w-xs p-6 bg-white rounded-xl shadow-xl border border-gray-100 text-sm text-gray-600 md:absolute md:top-8 md:left-0 md:translate-x-0 md:translate-y-0 md:w-64 md:p-4 md:shadow-lg md:border-blue-100 md:rounded-lg">
                            <ul className="list-disc pl-4 space-y-2">
                                <li>Precios por <strong>m²</strong> de aplicación.</li>
      ...
      <div className="flex flex-col gap-1 mb-4">
         <div className="flex items-center gap-2">
            <h4 className="font-bold text-gray-800 text-sm uppercase tracking-wide">Tipos de Fumigación</h4>
         </div>
         ...
         <div className="mt-2 flex items-center gap-2">
            <div className="relative inline-block w-full sm:w-64">
                <select
                    ...
                    onChange={(e) => {
                        if (e.target.value) {
                            addType(e.target.value as FumigationType);
                            e.target.value = '';
                        }
                    }}
                    defaultValue=""
...
export default FumigationPricingConfigurator;
