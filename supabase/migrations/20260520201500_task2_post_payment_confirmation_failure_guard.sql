...
CREATE OR REPLACE FUNCTION public.prepare_booking_payment_attempt_for_client(
  p_quote_id uuid,
  p_client_id uuid,
  p_checkout_ttl_minutes integer DEFAULT 30
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF FOUND THEN
    ...
    IF v_existing_attempt.status = 'reconciliation_required' THEN
      RETURN public.get_booking_payment_attempt_summary(v_existing_attempt.id);
    END IF;
  ...
  v_expires_at := now() + make_interval(mins => GREATEST(1, COALESCE(p_checkout_ttl_minutes, 30)));
