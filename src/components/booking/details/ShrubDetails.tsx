...
export const ShrubDetails = ({
    idx,
    group,
    allPhotos,
    isZoneAnalyzing,
    shrubUploads,
    handleShrubFileSelect,
    toggleShrubPhotoSelection,
    removeShrubPhoto,
    removeShrubGroup,
    analyzeShrubGroup,
    isAnalyzed,
    ...
    return (
        <div key={group.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
                ...
                <button onClick={() => removeShrubGroup(group.id)} className="text-gray-400 hover:text-red-500 p-2 hover:bg-red-50 rounded-lg transition-colors">
            ...
            <div className="mb-4">
                <ZonePhotoGallery
                    ...
                    onToggleSelection={(i: number) => toggleShrubPhotoSelection(group.id, i)}
                    onRemovePhoto={(i: number) => removeShrubPhoto(group.id, i)}
                    onAddPhotos={(e: any) => handleShrubFileSelect(group.id, e)}
            ...
            {isAnalyzed && (
                <div className="mt-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <ServiceResultCard
                        title="Poda de plantas y arbustos"
                        metrics={[
                            { label: 'Superficie', value: `${group.area} m²` },
                            { label: 'Tamaño', value: group.size || 'Pequeñas' }
                        ]}
