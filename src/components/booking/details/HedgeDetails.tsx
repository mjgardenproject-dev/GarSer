...
export const HedgeDetails = ({
    idx,
    zone,
    allPhotos,
    isZoneAnalyzing,
    hedgeUploads,
    handleHedgeFileSelect,
    toggleHedgePhotoSelection,
    removeHedgePhoto,
    removeHedgeZone,
    analyzeHedgeZone,
    isAnalyzed,
    ...
    if (isZoneAnalyzing) {
        return <AnalysisLoadingAnimation key={zone.id} message="Analizando setos..." />;
    }
    ...
    return (
        <div key={zone.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="mb-4">
                <ZonePhotoGallery
                    photos={allPhotos}
                    uploadingIndices={hedgeUploads[zone.id]}
                    selectedIndices={zone.selectedIndices}
                    analyzedIndices={zone.analyzedIndices}
                    isAnalyzing={isZoneAnalyzing}
                    isAnalyzed={isAnalyzed}
                    onToggleSelection={(i: number) => toggleHedgePhotoSelection(zone.id, i)}
                    onRemovePhoto={(i: number) => removeHedgePhoto(zone.id, i)}
                    onAddPhotos={(e: any) => handleHedgeFileSelect(zone.id, e)}
                ...
                <div className="mt-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <ServiceResultCard
                        ...
                        metrics={[
                            ...
                            { label: 'Altura', value: `${zone.height} m` },
