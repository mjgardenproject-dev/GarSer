...
| P-F1-1 | **Justo tras migrar:** hacer una reserva real pagada con Stripe (modo prueba) y comprobar en el SQL Editor que sus filas de `booking_blocks` tienen `assignee_id` = el jardinero | F1 | ✅ 2026-09-25: el chat hizo la reserva con la sesión de cliente (móvil) y el usuario pagó con la tarjeta de prueba (Stripe de producción en modo prueba, `pk_test_` comprobado). Reserva `0129ad33` pendiente, 05/10 9:00–12:00, 45 € + 5,63 € de gestión; `booking_blocks` 9, 10 y 11 con `assignee_id` = el jardinero; esas horas pasan a no disponibles; 1 `booking_items`; «Mis reservas» la enseña |
| P-F1-2 | El jardinero propone una hora más en una reserva pendiente y el cliente acepta: la reserva se confirma y la agenda crece | F1 | ✅ 2026-09-26 (chat; sesiones de jardinero y de cliente, móvil): el jardinero propuso 60 € y 4 h en la reserva `0129ad33`; el cliente la vio con motivo y fin a las 13:00 y la aceptó → `confirmed`, 60 €, fin 13:00, `booking_blocks` 9–12 con el jardinero, la hora 12 pasa a ocupada. Ver H-36 |
| P-F1-3 | Cancelar esa reserva: las horas vuelven a estar libres en la web | F1 | ✅ 2026-09-26: el cliente la canceló (aviso de que los 5,63 € no se devuelven) → `cancelled`, 0 bloques, 9–16 libres otra vez y la web vuelve a ofrecer 9–14 |
