...
| T13 | fitosanitarios (2026-09-12) | Menor: **«Solicitudes de Reserva» del profesional muestra «Retirada de restos incluida» aunque la reserva llegue con `wasteRemoval: false`.** Se vio al corregir el hallazgo #10 de fitosanitarios: el servicio dejó de preguntar y de declarar la retirada (ahora manda `false`), y el panel la sigue anunciando igual. El componente es compartido, así que en la rama de fitosanitarios solo se corrigió el ORIGEN del dato, no el panel. Cualquier servicio que declare `wasteRemoval: false` lo reproduce. | `src/components/gardener/BookingRequestsManager.tsx` (línea del detalle del servicio, no condicionada al flag) | los 7 servicios |
